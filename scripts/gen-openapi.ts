...
let eligible = packaged.filter((p) => {
  ...
  const specPath = join(SPEC_DIR, `${p.id}.yaml`);
  return !existsSync(specPath) || isGenerated(specPath);
});
...
const skipped = packaged.filter((p) => {
  ...
  if (!existsSync(join(ROOT, "packages", p.id, "src/api.ts"))) return false;
  const specPath = join(SPEC_DIR, `${p.id}.yaml`);
  return existsSync(specPath) && !isGenerated(specPath) && !FORCE;
});
...
if (!CHECK_MODE && !existsSync(SPEC_DIR)) {
...
for (const p of eligible) {
  ...
  const outPath = join(SPEC_DIR, `${p.id}.yaml`);
