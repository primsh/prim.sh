...
import { flatTasks, loadTasks } from "./lib/tasks.js";
...
function checkGates() {
  ...
  for (const gate of V0_GATES) {
    // biome-ignore lint/style/noNonNullAssertion: gate.id always exists in gateStatus map
    ...
    if (status === "done") {
      ...
      const isParallelActive = firstActive?.parallelWith?.includes(gate.id) ?? false;
      if (isParallelActive) {
