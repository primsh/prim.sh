...
email.sh gives agents their own email addresses with zero human setup:

- **Create mailbox** — `POST /v1/mailboxes` → agent gets `agent-7xk@email.sh` instantly
...
- **Custom domains** — Bring your own domain or use `*.email.sh` subdomains
...
Agent
    ↓
email.sh API (Hono + x402 middleware)
    ↓
┌─────────────────────────────────────┐
│  email.sh wrapper                    │
│                                      │
...
When email.sh creates a mailbox, it also obtains and caches an OAuth token for that user so it can make JMAP calls on the mailbox's behalf.
...
POST   /v1/mailboxes                     # Create mailbox
  Request:  { "domain": "email.sh", "ttl": 86400 }   ← optional, defaults shown
...
Under the hood: email.sh authenticates as the mailbox user via JMAP OAuth, calls `Email/set` (create draft) + `EmailSubmission/set` (send) in a single JMAP batch request.
...
When email arrives at the mailbox, email.sh:
...
POST   /v1/domains
  ...
  Response: {
    ...
    "status": "pending_dns",
    "dns_records": [
      { "type": "MX", "name": "mail.myagent.com", "value": "email.sh", "priority": 10 },
      { "type": "TXT", "name": "mail.myagent.com", "value": "v=spf1 include:email.sh ~all" },
...
```
VPS (Hetzner)
├── email.sh API (Hono on Bun, port 3001)
├── Stalwart (Docker)
│   ├── SMTP (25, 587, 465)
│   ├── JMAP (443)
│   └── Management API (8080, internal only)
├── SQLite (email.sh state: mailbox→wallet mapping, webhooks, TTLs)
└── Reverse proxy (Caddy or nginx)
    ├── api.email.sh → email.sh API
    └── mail.email.sh → Stalwart SMTP/JMAP
```
