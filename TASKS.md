...
| I-3 | Coverage gate: vitest coverage runs succeed but `coverage-summary.json` is never written — needs `reportsDirectory` set in each vitest.config.ts so output lands in `packages/<id>/coverage/`. Without it the gate runner's coverage threshold check always warns instead of enforcing. 11 packages to fix. | packages/*/vitest.config.ts | — | pending |
