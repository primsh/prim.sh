  ...
  group("store.sh health", () => {
    ...
    check(res, {
      ...
      "GET / → has service field": (r) => {
        try { return JSON.parse(r.body).service === "store.sh"; } catch { return false; }
      },
      "GET / → status ok": (r) => {
...
const PAID_ENDPOINTS = [
  // Bucket CRUD
  { method: "POST",   path: "/v1/buckets",                        body: JSON.stringify({ name: "load-test-bucket" }),         label: "POST /v1/buckets" },
  ...
  { method: "DELETE", path: "/v1/buckets/test-bucket-id",         body: null,                                                  label: "DELETE /v1/buckets/:id" },
  // Object CRUD
  { method: "PUT",    path: "/v1/buckets/test-bucket-id/objects/test-key.txt", body: "hello load test", label: "PUT /v1/buckets/:id/objects/*" },
  { method: "GET",    path: "/v1/buckets/test-bucket-id/objects",              body: null,              label: "GET /v1/buckets/:id/objects" },
  { method: "GET",    path: "/v1/buckets/test-bucket-id/objects/test-key.txt", body: null,              label: "GET /v1/buckets/:id/objects/*" },
  { method: "DELETE", path: "/v1/buckets/test-bucket-id/objects/test-key.txt", body: null,              label: "DELETE /v1/buckets/:id/objects/*" },
  // Quota
  { method: "GET",    path: "/v1/buckets/test-bucket-id/quota",   body: null,                                                  label: "GET /v1/buckets/:id/quota" },
];
...
export function paidScenario() {
  ...
  group("store.sh paid", () => {
    ...
    if (is5xx) {
      console.error(`[store.sh] 5xx on ${ep.label}: status=${res.status} body=${res.body.slice(0, 200)}`);
    }
  });
