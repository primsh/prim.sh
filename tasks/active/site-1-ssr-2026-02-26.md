...
27 HTML files share ~90% identical CSS and structure. The only per-page differences are: accent color, name, tagline, curl examples, and section content. The CSS has already drifted (spawn uses `--green`/`--green-dim` instead of `--accent`). Updating any shared element (CSS, footer, nav) requires editing 27 files.
...
3. **Write serve.ts** — YAML loader + template renderer + HTTP server. Same routing as serve.py. Test locally: `bun run site/serve.ts`.
...
- `bun run site/serve.ts` serves all 27 prim pages locally with correct content
