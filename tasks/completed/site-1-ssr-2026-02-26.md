...
31 HTML files share ~90% identical CSS and structure. The only per-page differences are: accent color, name, tagline, curl examples, and section content. The CSS has already drifted (spawn uses `--green`/`--green-dim` instead of `--accent`). Updating any shared element (CSS, footer, nav) requires editing 31 files.
...
> **Design note (fix after initial implementation):** accent colors are a site-level design decision, not a primitive concern. The correct long-term location is a `site/config.yaml` mapping `id → accent/accent_dim/accent_glow`, with template joining prim content + site config at render time. This also absorbs `card_class` from I-1. For now, per-prim YAML is acceptable to unblock dev — refactor color ownership to site config in a follow-up task.
