...
4. **Clone the repository (or copy deploy/email/):**
   ```bash
   git clone https://github.com/your-org/agentstack.git
   cd agentstack/deploy/email
   ```

   Or, if copying files manually:
   ```bash
   mkdir -p deploy/email
   scp docker-compose.yml root@<VPS_IP>:/root/deploy/email/
   scp .env.example root@<VPS_IP>:/root/deploy/email/
   ```
...
| 465 | SMTPS | Submission SSL (email.sh → Stalwart) | Public (port forwarded) |
...
- `MAIL_DOMAIN`: Domain for email.sh (configured in R-2)
