...
/**
 * store.sh — Tier 3 e2e-local test
 *
 * Boots store.sh locally, sends real HTTP requests with x402 on testnet.
 * Usage: source scripts/.env.testnet && bun e2e/tests/store.generated.ts
 */
...
async function start(): Promise<void> {
  ...
  const tmpHome = `/tmp/prim-e2e-store-${Date.now()}`;
  mkdirSync(tmpHome, { recursive: true });
  mkdirSync(`${tmpHome}/data`, { recursive: true });
  proc = spawn("bun", ["run", "packages/store/src/index.ts"], {
    env: {
      ...process.env,
      PORT: String(PORT),
...
console.log(`\nstore.sh e2e-local (testnet, ${URL})\n`);
...
try {
  ...
  await test("POST /v1/buckets → 402 without payment", async () => {
    const res = await fetch(`${URL}/v1/buckets`, { method: "POST", body: "{}" });
    assert(res.status === 402, `expected 402, got ${res.status}`);
  });

  await test("POST /v1/buckets → paid request", async () => {
    const res = await primFetch(`${URL}/v1/buckets`, {
      method: "POST",
