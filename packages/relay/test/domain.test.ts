...
describe("custom domains (R-9)", () => {
  ...
  describe("registerDomain", () => {
    ...
    it("rejects reserved domain relay.prim.sh", async () => {
      const result = await registerDomain({ domain: "relay.prim.sh" }, WALLET_A);
  ...
  describe("verifyDomain", () => {
    it("sets status to active when all DNS checks pass", async () => {
      ...
      (verifyDns as ReturnType<typeof vi.fn>).mockResolvedValue({
        allPass: true,
        mx: { pass: true, expected: "mail.relay.prim.sh", found: "mail.relay.prim.sh" },
        spf: { pass: true, expected: "include:relay.prim.sh", found: "v=spf1 include:relay.prim.sh -all" },
    ...
    it("returns pending with verification_results when MX missing", async () => {
      ...
      (verifyDns as ReturnType<typeof vi.fn>).mockResolvedValue({
        allPass: false,
        mx: { pass: false, expected: "mail.relay.prim.sh", found: null },
        spf: { pass: true, expected: "include:relay.prim.sh", found: "v=spf1 include:relay.prim.sh -all" },
        dmarc: { pass: true, expected: "v=DMARC1", found: "v=DMARC1; p=quarantine" },
      });
    ...
    it("returns pending when SPF missing", async () => {
      ...
      (verifyDns as ReturnType<typeof vi.fn>).mockResolvedValue({
        allPass: false,
        mx: { pass: true, expected: "mail.relay.prim.sh", found: "mail.relay.prim.sh" },
        spf: { pass: false, expected: "include:relay.prim.sh", found: null },
        dmarc: { pass: true, expected: "v=DMARC1", found: "v=DMARC1; p=quarantine" },
      });
  ...
  describe("createMailbox with custom domain", () => {
    ...
    it("still allows relay.prim.sh domain", async () => {
      (createPrincipal as ReturnType<typeof vi.fn>).mockResolvedValue(42);
