...
describe("crypto", () => {
  ...
  afterEach(() => {
    process.env.RELAY_ENCRYPTION_KEY = undefined;
  });
  ...
  it("throws when RELAY_ENCRYPTION_KEY is missing", () => {
    process.env.RELAY_ENCRYPTION_KEY = "";
    expect(() => getEncryptionKey()).toThrow("RELAY_ENCRYPTION_KEY environment variable is required");
  });

  it("throws when RELAY_ENCRYPTION_KEY is wrong length", () => {
    process.env.RELAY_ENCRYPTION_KEY = "abcd";
